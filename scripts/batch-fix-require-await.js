...
/**
 * Batch fix require-await errors by removing async from functions with no await
 * Reads ESLint output and fixes files directly
 */
...
for (const [file, lineNumbers] of filesToFix) {
  try {
    ...
    for (const lineNum of lineNumbers) {
      ...
      // Check if function body has await (look ahead up to 100 lines)
      let hasAwait = false
      let braceCount = 0
      ...
      for (let i = idx; i < Math.min(idx + 100, lines.length); i++) {
        ...
        // Check for await (not in comments)
        if (foundOpenBrace && checkLine.includes('await') && !checkLine.trim().startsWith('//') && !checkLine.trim().startsWith('*')) {
          hasAwait = true
          break
        }
    ...
    if (modified) {
      ...
      totalFixed++
      console.log(`✅ Fixed: ${file} (${lineNumbers.length} errors)`)
    }
  } catch (error) {
...
console.log(`\n✅ Fixed ${totalFixed} files, removed ${totalRemoved} async keywords`)
