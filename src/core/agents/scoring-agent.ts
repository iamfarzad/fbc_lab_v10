...
export async function scoringAgent(
  ...
  const { intelligenceContext: intelligenceContextRaw, conversationFlow, multimodalContext } = context
  ...
  const companySize = 'Unknown' // Company size not in schema, would need to extend
  steps.push({
